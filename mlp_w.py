...
class MLP:
        ...
        for n in range(niterations):
            ...
            for i in range(ndata):
                ...
                for j in range(len(soft)):
                    for k in range(len(soft)):
                        if j == k:
                            soft[j][k] = output[j] * (1-output[j])
                        else:
                            soft[j][k] = -output[j]*output[k]
                deltao_i = np.dot(output - targets[i], soft)
                deltao[i] = deltao_i
            deltao /= ndata
            # compute the derivative of the second hidden layer
            deltah2 = self.hidden2 * self.beta * (1 - self.hidden2) * (np.dot(deltao, np.transpose(self.weights3)))
            # compute the derivative of the first hidden layer

            deltah1 = self.hidden1[:, :-1] * self.beta * (1 - self.hidden1[:, :-1]) * (np.dot(deltah2, self.weights2))
    ...
    def forwardPass(self, inputs):
        ...
        self.hidden1 = 1.0 / (1.0 + np.exp(-self.hidden1 * self.beta))
        self.hidden1 = np.concatenate((self.hidden1, -np.ones((np.shape(inputs)[0], 1))), axis=1)  # adding bias
        ...
        self.hidden2 = 1.0 / (1.0 + np.exp(-self.hidden2 * self.beta))
        self.hidden2 = np.concatenate((self.hidden2, -np.ones((np.shape(self.hidden1)[0], 1))), axis=1)  # adding bias
